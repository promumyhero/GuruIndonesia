...
export default function StudentAssessmentsPage() {
  ...
  const filteredAssessments = assessments.filter((assessment) => {
    ...
    return searchMatch && typeMatch && subjectMatch && dateMatch;
  });

  return (
    <div className="container p-4 md:p-6">
      <Breadcrumb items={[{ label: "Nilai Saya", href: "/student/assessments" }]} />
      
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        <div className="flex items-center gap-2">
          <ClipboardList className="h-6 w-6 text-primary" />
          <h1 className="text-2xl md:text-3xl font-bold">Nilai Saya</h1>
        </div>
      </div>
      
      <Card className="mb-6">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg">Filter Nilai</CardTitle>
          <CardDescription>Cari dan filter data nilai</CardDescription>
        </CardHeader>
        <CardContent>
